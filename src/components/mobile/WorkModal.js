...
const WorkModal = ({isOpen, handleModal, slides, location, clickedSlide}) => {
    ...
    const works = [
        {
            ...
            contribution : "100%",
            mainImgSrc: mainImgTravelNote,
            imgSrcs: [imgTravelNote1, imgTravelNote2, imgTravelNote3, imgTravelNote4, imgTravelNote5, imgTravelNote6, imgTravelNote7]
        },
        {
            name: "꽃뜨 도자기카페",
            ...
            contribution : "100%",
            mainImgSrc: mainImgFlowerth,
            imgSrcs: [imgFlowerth1, imgFlowerth2, imgFlowerth3, imgFlowerth4]
        },
        {
            name: "Michael Kors",
            ...
            techStack: "HTML, CSS, JavaScript",
            contribution : "100%",
            mainImgSrc: mainImgMichaelKors,
            imgSrcs: [imgMichaelKors1, imgMichaelKors2, imgMichaelKors3, imgMichaelKors4]
        }
    ]

    const closeModal = (e) => {
        if(isOpen && e.target && e.target.className.includes('WorksContainer')){
    ...
    return (
        <div className={isOpen ?
            ...
            <div>
                <img src={work && work.mainImgSrc} className={styles.mainImg}></img>
                <div className={styles.informationBox}>
                    <div>
                        <h2>웹사이트명</h2>
                        <a href={work && work.url} target="_blank">
                            <p className={styles.link}>{work && work.name}<FaExternalLinkAlt size="14"/></p>
                        </a>
                    </div>
                    <div>
                        <h2>목적 및 내용</h2>
                    ...
                    {work && work.imgSrcs.map((imgSrc, id) => {
                        return(
                            <img key={id} src={imgSrc} className={styles.mainImg}></img>
                        )
                    })}
                </div>
            </div>
